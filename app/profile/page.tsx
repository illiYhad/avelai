...
export default function ProfilePage() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#09090b] flex items-center justify-center">
      <div className="text-zinc-500 animate-pulse">Loading Passport...</div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#09090b] text-zinc-100 p-4 md:p-6">
      <div className="max-w-5xl mx-auto space-y-4">
              ...
              <div className="flex items-center gap-2 mt-1 text-xs text-zinc-500">
                ...
                {(['online', 'dnd', 'offline'] as const).map(s => (
                  <button key={s} onClick={() => setStatus(s)}
                    className={`px-2 py-0.5 rounded border text-xs transition-all ${status === s ? statusConfig[s].color : 'border-zinc-700 text-zinc-600 hover:text-zinc-400'}`}>
                    {s}
                  </button>
                ))}
              </div>
        ...
        <div className="p-5 rounded-2xl bg-zinc-900/80 border border-zinc-800">
          ...
          <div className="relative flex items-center justify-center h-32 rounded-xl border border-amber-500/30 bg-gradient-to-br from-zinc-900 via-red-950/20 to-zinc-900 shadow-[0_0_30px_rgba(245,158,11,0.1)] overflow-hidden">
            ...
            <button className="absolute bottom-3 right-3 px-3 py-1 text-xs rounded-lg bg-zinc-800 border border-zinc-700 text-zinc-300 hover:bg-zinc-700 transition-all">
              Customize Card
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            { icon: <Shield className="w-5 h-5 text-emerald-400" />, label: 'Karma Score', value: `${profile?.karma_score ?? 100}`, color: 'text-emerald-400 border-emerald-800/40 bg-emerald-950/20' },
            { icon: <Trophy className="w-5 h-5 text-amber-400" />, label: 'Win Rate', value: `${profile?.win_rate ?? '0.00'}%`, color: 'text-amber-400 border-amber-800/40 bg-amber-950/20' },
