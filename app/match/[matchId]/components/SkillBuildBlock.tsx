...
const POS_COLORS: Record<string, string> = {
    'Pos 1': '#E8384F', 'Pos 2': '#2E9BFF', 'Pos 3': '#39FF6A', 'Pos 4': '#D63CE8', 'Pos 5': '#C8CDD4',
};

const HERO_DATA_MAP: Record<number, { name: string; shortName: string }> = {
    1: { name: 'Anti-Mage', shortName: 'antimage' }, 2: { name: 'Axe', shortName: 'axe' },
    6: { name: 'Drow Ranger', shortName: 'drow_ranger' }, 14: { name: 'Pudge', shortName: 'pudge' },
    22: { name: 'Zeus', shortName: 'zuus' }, 74: { name: 'Invoker', shortName: 'invoker' },
    76: { name: 'Outworld Destroyer', shortName: 'obsidian_destroyer' }, 84: { name: 'Ogre Magi', shortName: 'ogre_magi' },
    90: { name: 'Keeper of the Light', shortName: 'keeper_of_the_light' }, 93: { name: 'Slark', shortName: 'slark' },
    96: { name: 'Centaur Warrunner', shortName: 'centaur' }, 121: { name: 'Grimstroke', shortName: 'grimstroke' },
    137: { name: 'Primal Beast', shortName: 'primal_beast' },
};

const HERO_ABILITY_DETAILS: Record<number, { key: string; name: string }[]> = {
    1: [{ key: 'antimage_mana_break', name: 'Mana Break' }, { key: 'antimage_blink', name: 'Blink' }, { key: 'antimage_counterspell', name: 'Counterspell' }, { key: 'antimage_mana_void', name: 'Mana Void' }],
    6: [{ key: 'drow_ranger_frost_arrows', name: 'Frost Arrows' }, { key: 'drow_ranger_multishot', name: 'Multishot' }, { key: 'drow_ranger_silence', name: 'Gust' }, { key: 'drow_ranger_marksmanship', name: 'Marksmanship' }],
    14: [{ key: 'pudge_meat_hook', name: 'Meat Hook' }, { key: 'pudge_rot', name: 'Rot' }, { key: 'pudge_flesh_heap', name: 'Flesh Heap' }, { key: 'pudge_dismember', name: 'Dismember' }],
    22: [{ key: 'zuus_arc_lightning', name: 'Arc Lightning' }, { key: 'zuus_lightning_bolt', name: 'Lightning Bolt' }, { key: 'zuus_heavenly_jump', name: 'Heavenly Jump' }, { key: 'zuus_thundergods_wrath', name: "Thundergod's Wrath" }],
    76: [{ key: 'obsidian_destroyer_arcane_orb', name: 'Arcane Orb' }, { key: 'obsidian_destroyer_astral_imprisonment', name: 'Astral Imprisonment' }, { key: 'obsidian_destroyer_essence_flux', name: 'Essence Flux' }, { key: 'obsidian_destroyer_sanity_eclipse', name: "Sanity's Eclipse" }],
    84: [{ key: 'ogre_magi_fireblast', name: 'Fireblast' }, { key: 'ogre_magi_ignite', name: 'Ignite' }, { key: 'ogre_magi_bloodlust', name: 'Bloodlust' }, { key: 'ogre_magi_multicast', name: 'Multicast' }],
    90: [{ key: 'keeper_of_the_light_illuminate', name: 'Illuminate' }, { key: 'keeper_of_the_light_blinding_light', name: 'Blinding Light' }, { key: 'keeper_of_the_light_chakra_magic', name: 'Chakra Magic' }, { key: 'keeper_of_the_light_spirit_form', name: 'Spirit Form' }],
    93: [{ key: 'slark_dark_pact', name: 'Dark Pact' }, { key: 'slark_pounce', name: 'Pounce' }, { key: 'slark_essence_shift', name: 'Essence Shift' }, { key: 'slark_shadow_dance', name: 'Shadow Dance' }],
    96: [{ key: 'centaur_hoof_stomp', name: 'Hoof Stomp' }, { key: 'centaur_double_edge', name: 'Double Edge' }, { key: 'centaur_work_horse', name: 'Work Horse' }, { key: 'centaur_stampede', name: 'Stampede' }],
    121: [{ key: 'grimstroke_dark_artistry', name: 'Stroke of Fate' }, { key: 'grimstroke_ink_creature', name: "Phantom's Embrace" }, { key: 'grimstroke_spirit_walk', name: 'Ink Swell' }, { key: 'grimstroke_soul_chain', name: 'Soulbind' }],
    137: [{ key: 'primal_beast_onslaught', name: 'Onslaught' }, { key: 'primal_beast_trample', name: 'Trample' }, { key: 'primal_beast_uproar', name: 'Uproar' }, { key: 'primal_beast_pulverize', name: 'Pulverize' }],
};
...
export default function SkillBuildBlock({ players = [], heroIdToImg = {}, itemIdToName = {} }: any) {
    ...
    const getHeroImg = (heroId: number) => {
        ...
        if (shortName) return `https://cdn.cloudflare.steamstatic.com/apps/dota2/images/dota_react/heroes/${shortName}.png`;
        return '';
    };
    ...
    const renderHeroBuildMatrix = (p: any) => {
        ...
        const heroItems = [
            p.items?.[0] ?? p.item_0 ?? p.item0 ?? 0, p.items?.[1] ?? p.item_1 ?? p.item1 ?? 0,
            p.items?.[2] ?? p.item_2 ?? p.item2 ?? 0, p.items?.[3] ?? p.item_3 ?? p.item3 ?? 0,
            p.items?.[4] ?? p.item_4 ?? p.item4 ?? 0, p.items?.[5] ?? p.item_5 ?? p.item5 ?? 0,
        ];
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 border-b border-neutral-800/80 pb-3">
                    <div className="flex items-center gap-3">
                        <div className="h-10 w-16 overflow-hidden border border-neutral-700 bg-neutral-900 shrink-0">
                            {heroImg ? (
                                <img
                                    src={heroImg} alt={heroDisplayName} className="h-full w-full object-cover"
                                    onError={(e) => {
                                        const target = e.target as HTMLImageElement;
                                        const shortName = HERO_DATA_MAP[p.heroId]?.shortName;
                                        if (shortName && !target.src.includes(shortName)) target.src = `https://cdn.cloudflare.steamstatic.com/apps/dota2/images/dota_react/heroes/${shortName}.png`;
                                    }}
                                />
                    ...
                    <div className="flex items-center gap-1 bg-[#07070C] p-1 border border-neutral-800">
                        {heroItems.map((itemId, i) => {
                            ...
                            return (
                                <div key={i} title={cleanItemName || (itemId ? `Item #${itemId}` : 'Empty Slot')} className="h-7 w-10 border border-neutral-800 bg-neutral-900 overflow-hidden flex items-center justify-center">
                                    {itemUrl ? (
                                        <img src={itemUrl} alt="item" className="h-full w-full object-cover" onError={(e) => {
                                            const target = e.target as HTMLImageElement;
                                            if (cleanItemName && !target.src.includes('api.opendota.com')) target.src = `https://api.opendota.com/apps/dota2/images/dota_react/items/${cleanItemName}.png`;
                        ...
                        <div className="space-y-1">
                            {abilitySlots.map((slot, sIdx) => (
                                <div key={sIdx} className="grid grid-cols-[130px_repeat(25,1fr)] items-center gap-1">
                                    <div title={slot.name} className="flex h-7 items-center gap-1.5 border border-neutral-800 bg-[#161622] px-1 overflow-hidden">
