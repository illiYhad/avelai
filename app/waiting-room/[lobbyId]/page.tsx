...
export default function WaitingRoomPage({ params }: PageProps) {
  ...
  useEffect(() => {
    ...
    // 1. ดึงข้อมูล Lobby จริงจาก Supabase
    ...
    // 2. Realtime Subscription อัปเดตสถานะห้องสด
    ...
    return (
      <div className="min-h-screen bg-[#07090E] text-rose-400 flex flex-col items-center justify-center font-mono space-y-4">
        ...
        <button
          ...
          className="px-6 py-2 border border-rose-500/50 hover:bg-rose-500/10 text-white rounded-lg text-xs tracking-widest uppercase transition-all"
        >
          ← Return to Arena
        </button>
      </div>
  ...
  // คำนวณ AVG Form Level หรือ Elo
  const avgLevelA = formation.averageFormLevelTeamA ?? (teamA.length > 0 ? (teamA.reduce((acc: number, p: any) => acc + (p.tierProfile?.formLevel || 10), 0) / teamA.length).toFixed(1) : 0);
  const avgLevelB = formation.averageFormLevelTeamB ?? (teamB.length > 0 ? (teamB.reduce((acc: number, p: any) => acc + (p.tierProfile?.formLevel || 10), 0) / teamB.length).toFixed(1) : 0);
        ...
        <main className="space-y-6">
          ...
          <section className="bg-[#0B0F17]/80 border border-[#00D4FF]/30 p-4 rounded-xl shadow-xl space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3">
              {teamA.map((player: any, idx: number) => (
                <ProfileCard
                  key={player.userId || idx}
                  player={{
                    id: player.userId,
                    display_name: player.userId.slice(0, 10),
                    elo: (player.tierProfile?.formLevel || 10) * 100,
                    role: `POS ${player.assignedPosition || idx + 1}`,
                    card_rarity: 'CYBER_HOLO',
                    rank_tier: player.tierProfile?.tierCode || 'T10'
                  }}
                  teamType="radiant"
                />
              ))}
          ...
          <section className="bg-[#0B0F17]/80 border border-[#C9A84C]/30 p-4 rounded-xl shadow-xl space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3">
              {teamB.map((player: any, idx: number) => (
                <ProfileCard
                  key={player.userId || idx}
                  player={{
                    id: player.userId,
